...
- **Primary Code:** `storage-service.js` - STORAGE_KEYS and DEFAULTS definitions, including migration metadata keys.
...
- Startup migration writes schema state to both `chrome.storage.local` and IDB `meta/schemaVersion`; debug clients can query this through `GET_MIGRATION_STATUS`.
