...
- **Tier 1 - In-Memory:** `faviconCache` at `content.js:37` for instant lookup. At content-script startup, `scheduleFaviconMemoryPrefetch()` triggers a lightweight IDB-only prefetch (no network) that populates this cache in small batches (80 domains per batch, `requestIdleCallback` between batches) so the first overlay open can skip the IDB round-trip. Public domains may still share root-domain aliases, but private/local services keep their exact `host:port` key so different localhost/LAN ports do not collapse into one in-memory icon.
