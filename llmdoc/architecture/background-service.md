...
- `background-messages.js` (`handleMessage`): Message router. Switch-based dispatcher for message actions (including `TRACK_BOOKMARK_OPEN`), favicon operations via IndexedDB, persisted favicon success/failure state reads, plus SW in-memory LRU for browser-provided favicons. Persisted success records are reused without the old read-time TTL, while failure records carry `retryAt` cooldown semantics so later searches can retry. Browser favicon responses now also expose placeholder metadata to keep default browser globe results out of the long-lived success cache. The SW memory cache no longer uses TTL; entries stay available until evicted by LRU capacity or lost on Service Worker restart. For private hosts (localhost/IP/internal suffixes), browser favicon fetch still uses a longer timeout and keeps raw host keys instead of collapsing them. SVG/data-URL monogram placeholders from browser favicon are also rejected for private hosts, and `CLEAR_FAVICON_CACHE` now clears SW memory cache, deletes persisted `favicon:*` entries, and broadcasts cache invalidation to content scripts.
