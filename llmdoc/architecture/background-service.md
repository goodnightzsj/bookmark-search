...
- `background-data.js` (`refreshBookmarks`, `applyBookmarkEvents`, `searchBookmarks`, `loadInitialData`, `getWarmupDomainMap`, `recordBookmarkOpen`): Data management layer. A single runtime state container keeps `documents` as the primary in-memory source plus a derived bookmark compatibility view and bookmark-id index for boundary logic. Search uses memory-cached `lastSyncTime` and TTL to avoid per-search storage reads and can lazily hydrate documents from IndexedDB before falling back to the native Chrome bookmarks API. The module also handles concurrency control, incremental/full sync logic, stale-cache TTL trigger, warmup domain prioritization, and documents fingerprint caching for consistency checks. History updates are in-memory only; persistence is handled by `saveToStorage`.
