...
4. **Incremental Update:** `background-data.js:applyBookmarkEventsOnce()` mutates the runtime state container in memory, uses a pre-built `docIndexById` Map for O(1) document lookup during changed/moved events, keeps the derived bookmark index aligned, and auto-falls back to full refresh if >200 events or folder-level changes.
